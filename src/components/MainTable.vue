<template>
    <table class="table table-pin-rows w-max table-fixed">
        <thead>
            <tr>
                ...
                <th :class="` text-ellipsis overflow-hidden whitespace-nowrap`" v-for="player in PlayerStore.players"
                    :key="player.id">
        ...
        <tr v-for="card in CardStore.cards" :key="card.id">
            <td :class="getTrClasses(card.id)">
                {{ card.name }}
            </td>
            <td v-for="player in PlayerStore.players" :key="player.id" :class="getTdClasses(player.id, card.id)"
                ...
                <div class="flex flex-col">
                    <div class="absolute top-0 left-0 w-full flex flex-row justify-start">
                        <template v-for="(question, index) in QuestionStore.questions" :key="question.id">
                            <span class="text-xs tooltip tooltip-error hover:z-50" :data-tip="$t('does-not-have-this-card-with-question', {
                                player: player.name,
                                questionNumber: index + 1,
                            })" v-if="question.playersThatDidntHaveAnythingIds.includes(player.id) &&
    ...
    ">
                    ...
                    <div>
                        <button class="btn btn-sm btn-square hover:btn-success btn-ghost tooltip hover:z-50 normal-case"
                            :data-tip="$t('has-this-card')"
                            @click="() => upsertPlayerCardRelation(player.id, card.id, true)">
                            <Icon name="md-check"></Icon>
                        </button>
                        <button class="btn btn-sm btn-square hover:btn-error btn-ghost tooltip hover:z-50 normal-case"
                    ...
                    <div class="absolute bottom-0 right-0 w-full flex flex-row justify-end">
                        <template v-for="(question, index) in QuestionStore.questions" :key="question.id">
                            <span class="tooltip tooltip-success text-xs hover:z-20" :data-tip="$t('showed-this-card-to', {
                                player: player.name,
                                ...
                                " v-if="question.answeringPlayerId === player.id &&
        (question.suspectCardId === card.id ||
            question.weaponCardId === card.id ||
            question.locationCardId === card.id)
        ">
...
function getTrClasses(cardId: string) {
    ...
    if (relations.filter(r => r.value).length === 0 && relations.length === PlayerStore.players.length) {
