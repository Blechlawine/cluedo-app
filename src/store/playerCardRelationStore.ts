...
import { PlayerCardRelationInput, PlayerCardRelationOutput, PlayerCardRelationValidator } from "../types/validators";
interface IState {
...
const usePlayerCardRelations = defineStore<"playerCardRelations", IState, TGetters, IActions>("playerCardRelations", {
    state: () => ({
        playerCardRelations: [],
    }),
    persist: true,
    getters: {
        getByPlayerIdAndCardId: (state) => (playerId, cardId) =>
            state.playerCardRelations.find((p) => p.playerId === playerId && p.cardId === cardId),
    },
    actions: {
        upsert(playerCardRelation) {
            ...
            if (parsed.success) {
                ...
                let temp = this.playerCardRelations.find((c) => c.playerId === data.playerId && c.cardId === data.cardId);
                if (temp) {
